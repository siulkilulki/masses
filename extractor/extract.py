...
class Extractor:
    ...
    def extract(self, search_space=None):
        if not search_space:
            search_space = self.content.lower()
        header_match = re.search(self.header, search_space)
        if not header_match:
            return None
        search_space = search_space[header_match.end():]

        sunday_title_match = re.search(self.sunday_title, search_space)
        if not sunday_title_match:
            return None
        if re.search(self.header, search_space[:sunday_title_match.start()]
                     ):  # found header closer to sunday title
        ...
        if not re.search(self.sunday_masses, sunday_masses_hours, re.DOTALL):
            return None
...
def process_parish(reader):
    ...
    #     """.format(
    #         match.group('header'),
    #         match.group('sunday_title'),
    #         match.group('sunday_masses'),
    #         match.group('everyday_title'), match.group('everyday_masses')))
    #     print(match.group(0) + '\n\n')


def extract(page):
    extractor = Extractor(page['content'])
    extractor.extract()
